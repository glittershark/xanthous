{ pkgs ? (import ./nix/nixpkgs.nix { }), ... }:

let
  gitignoreSource = import (import ./nix/sources.nix { })."gitignore.nix" { };
  ignore = gitignoreSource.gitignoreFilter ./.;
  src = builtins.path {
    name = "xanthous-source";
    path = ./.;
    filter = path: type:
      !(type == "directory" && builtins.baseNameOf path == "server")
      && !(type == "directory" && builtins.baseNameOf path == "docs")
      && (ignore path type || builtins.baseNameOf path == "package.yaml");
  };
  # generated by cabal2nix
  # goodbye ghc8107 i'm going to miss you
  basePkg = (pkgs.haskell.packages.ghc8107 or pkgs.haskellPackages).callPackage
    ./pkg.nix { };

in pkgs.haskell.lib.overrideCabal basePkg (default: {
  inherit src;
  # version = "canon";
  configureFlags = [ "--ghc-option=-Wall --ghc-option=-Werror" ]
    ++ (default.configureFlags or [ ]);
})
